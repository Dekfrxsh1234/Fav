        ...
        try:
            ...
            for game_id, start_time_str in active_games:
                try:
                    ...
                    if now - start_time > timedelta(minutes=GAME_TIMEOUT_MINUTES):
                        ...
                        if state and state["status"] == "active":
                            ...
                            try:
                                ...
                                logger.error(f"Failed to send timeout DM for game {game_id}: {e}", exc_info=True)
                except Exception as e:
                    logger.warning(f"Error processing game {game_id} in timeout checker: {e}", exc_info=True)
        except Exception as e:
            logger.error(f"Unhandled error in check_expired_games loop: {e}", exc_info=True)
